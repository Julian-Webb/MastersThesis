...
After dataset cleaning, the analysis pipeline began with a comprehensive preprocessing stage.
This included timestamp localization to ensure all \gls{eeg} recordings were temporally aligned, accounting for time zone differences and \gls{dst} changes.
Artifacts in the \gls{eeg} signals, such as extreme amplitude events or technical noise, were systematically detected and removed to prevent contamination of downstream analyses.
The continuous \gls{eeg} data were then segmented into short, fixed-length windows (\textit{segments}) and longer, non-overlapping windows (\textit{clips}), with careful labeling of intervals relative to seizure events (e.g., preictal, interictal).
The dataset was partitioned into training and test sets, ensuring sufficient representation of both periictal and interictal periods for robust model development and evaluation.

Feature extraction was performed on each segment, yielding a set of quantitative descriptors that captured both temporal and spectral properties of the \gls{eeg}.
...
Two main model architectures were employed: a \gls{cnn} that operated on raw \gls{eeg} segments, and an ensemble of \glspl{fb-mlp} that utilized the extracted features.
...
Model evaluation was conducted using both standard machine learning metrics and \glspl{ebm} tailored to the clinical context of seizure prediction.
...
Thresholds for issuing warnings were optimized to balance \gls{eb} sensitivity and specificity, and all evaluations were performed separately on training and test sets to assess generalization.

To further understand the temporal dynamics of seizure risk, cycle extraction was performed on the feature time series.
...
Finally, the propensity evaluation framework integrated these analyses to determine whether a model’s \glspl{fp} were meaningfully aligned with periods of increased seizure risk.
