...
The source code of this project is publicly available at \url{https://github.com/Julian-Webb/SeizurePredictionThesis}.

\chapter{Phase Histograms}\label{ch:phase-histograms}

This section contains the circular phase histograms per feature for all patients and event types (seizures, \gls{cnn} \glspl{fp}, ensemble \glspl{fp}), based on \cref{sec:cycle-extraction}
...
\end{itemize}
The mean angle is denoted after the feature name and also shown as a red arrow.
...
For each feature, the plot shows the original feature over time (original signal) and the filtered signal, after using the non-causal multidien filter (\cref{subsec:cycle-filtering}).
