...
The primary goal was to identify and quantify the rhythmic patterns of seizure occurrences and predictions in the time series data.
...
Within each fillable gap, imputation was performed independently per feature column by sampling adjacent segment values.
For a gap of length $L$, feature value samples were taken from neighboring segments before and after the gap within a window of size $W=\max(L, N_{\min})$, where $N_{\min}=100$ is the minimum sampling span.
The sampled values were then randomly assigned within the gap.
Additionally, each sampled value was perturbed by a multiplicative factor in the interval $[1-\delta,\;1+\delta]$, where $\delta=0.5$ is the maximum distortion percentage.
...
Only \glspl{fp} from valid clips in the test set were included, determined using the optimal threshold from \cref{subsec:threshold-optimization}.
...
Feature-based phase locking can be affected by artifacts present in the \gls{eeg} signal, including myogenic activity and technical noise.
...
Specifically, a 10th order Butterworth bandpass filter in \gls{sos} form was used with a period range of 5--50 days, and a sampling frequency corresponding to the segment duration.
...
To determine whether two groups of circular data, i.e., event phases from two event types, had significantly different mean directions, a non-parametric permutation test was used.
...
Since the Rayleigh and circular tests were performed for each feature (15 p-values per test), results were corrected for \gls{fdr} using the \gls{bh} procedure, yielding \gls{fdr}-adjusted p-values (also referred to as q-values) for each test.
