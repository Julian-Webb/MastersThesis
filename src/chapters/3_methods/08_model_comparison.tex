...
To quantify the tendency of false predictions to co-occur across models, the \gls{cope} \parencite{muller_coherent_2022} was calculated between the ensemble and \gls{cnn} scores for each patient.
...
The \textit{prediction error} vector of model $i$ is defined as $\vec{e}_i=\lvert\vec{p}_i-\vec{L}\rvert$ (analogously for model $j$).
The \textit{weight} $w_k$ for each sample $k$, the \textit{weighted mean} $\langle z\rangle_w$, and \textit{weighted standard deviation} $\sigma_w(z)$ are defined as:
